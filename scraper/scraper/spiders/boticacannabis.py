...
class ProductSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        categorias = response.css("nav#mk-main-navigation ul#menu-menu-principal li")
        for link_categoria in categorias:
            ...
            if url_category is not None:
                ...
                response.meta['shop'] = 'https://www.boticacannabis.cl'
                response.meta['shop_name'] = 'boticacannabis.cl'
                yield response
    ...
    def parse_category(self, response):
        ...
        list_product = response.css("div.theme-content  ul.products li div.product-loop-thumb a")
    ...
    def parse_product(self,response):
        ...
        categ = response.xpath('.//div[@class="mk_product_meta"]/span[@class="posted_in"]/a/text()').extract()
        category = None
        for a in categ:
        ...
        if True:
            ...
            name = response.xpath('.//h1[@class="single_product_title entry-title"]/text()').re_first('\w.*')
            ...
            try:
                description = ""
                description1 = response.xpath('.//div[@class="summary entry-summary mk-single-product"]/div[@itemprop="description"]').extract_first()
                description = re.sub("<div.*?>","",description1)
                description = re.sub("</div.*?>","",description)
            except:
                description = ""
            try:
                t = response.xpath('.//span[@class="amount"]/text()').re_first('\w.*')
                ti = t.split('.')
                total = ""
                for tii in ti:
                    total = total + str(tii)
        ...
        #             list_img_t = response.css("div.images")
        #
        #             print("*****************")
        #             print("*****************")
        #             print("*****************")
