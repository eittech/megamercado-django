...
class ProductSpider(scrapy.Spider):
    ...
    def parse(self, response):
        categorias = response.css("ul.product-categories li a")
        for link_categoria in categorias:
            ...
            try:
                ...
                response.meta['shop'] = 'http://www.cultivo.cl/shop/'
                response.meta['shop_name'] = 'cultivo.cl'
                yield response
            except:
                print("url no valida 1")
            # yield scrapy.Request(url=url_category, callback=self.parse_category)

    def parse_category(self, response):
        list_product = response.css("ul.products div.product-image a")
    ...
    def parse_product(self,response):
        ...
        categ = response.xpath('.//span[@class="posted_in"]/a/text()').extract()
        print("####################################")
        print("####################################")
        print("####################################")
        print(categ)
        ...
        if True:
            ...
            try:
                ...
                brand = None
            description = response.xpath('.//div[@id="tab-description"]/p/text()').re_first('\w.*')
            category = category
            ...
            if Product_exist:
                ...
                try:
                    if total > 0:
                        ...
                        print("No Se actualizo el precio es igual a 0")
                except:
                    product_error = True
