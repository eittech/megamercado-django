...
class ProductSpider(scrapy.Spider):
    name = "growshopesencial"

    def start_requests(self):
        urls = [
            "http://growshopesencial.cl/index.php",
        ]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        categorias = response.css("div#block_top_menu li")
        for link_categoria in categorias:
            ...
            response.meta['shop'] = 'http://growshopesencial.cl/'
            response.meta['shop_name'] = 'growshopesencial.cl'
            yield response
    ...
    def parse_category(self, response):
        list_product = response.css("ul#product_list div.product-container a.product_img_link")
        name_category = response.meta['name_category_safe']
        shop_url = response.meta['shop']
        shop_name = response.meta['shop_name']
