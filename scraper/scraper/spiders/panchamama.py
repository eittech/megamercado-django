...
class ProductSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        categorias = response.css("ul.product-categories li")
        for link_categoria in categorias:
            ...
            if url_category is not None:
                ...
                response.meta['shop'] = 'https://panchamama.cl/tienda/'
                response.meta['shop_name'] = 'panchamama.cl'
                yield response
    ...
    def parse_category(self, response):
        ...
        list_product = response.css("div.shop-container div.product-small p.product-title a")
    ...
    def parse_product(self,response):
        ...
        categ = response.xpath('.//nav[@class="woocommerce-breadcrumb breadcrumbs"]/a/text()').extract()
        print("################ CATEGORIAS ####################")
        print(categ)
        category = None
        for a in categ:
        ...
        name = response.xpath('.//h1[@class="product-title entry-title"]/text()').re_first('\w.*')
        ...
        # tax =
        try:
            t = response.xpath('.//div[@class="product-info summary col col-fit entry-summary product-summary text-left form-flat"]/div[@class="price-wrapper"]')
            c = t.xpath('.//p[@class="price product-page-price "]/span[@class="woocommerce-Price-amount amount"]')
            t = c[0].xpath('text()').re_first('\w.*')
            if t is None:
