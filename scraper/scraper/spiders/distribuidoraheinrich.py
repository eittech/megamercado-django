...
class ProductSpider(scrapy.Spider):
    name = "distribuidoraheinrich"

    def start_requests(self):
        urls = [
            "http://www.distribuidoraheinrich.cl/category/pilas-alcalinas/duracell/",
        ]
        for url in urls:
    ...
    def parse(self, response):
        categorias = response.css("div#modulo-accordion li a")
        for link_categoria in categorias:
            ...
            try:
                ...
                response.meta['shop'] = 'http://www.distribuidoraheinrich.cl/'
                response.meta['shop_name'] = 'distribuidoraheinrich.cl'
                yield response
            except:
                print("url no valida")
    ...
    def parse_category(self, response):
        list_product = response.css("div.entrada-catalogo p a")
    ...
    def parse_product(self,response):
        ...
        categ = response.xpath('.//p[@id="breadcrumbs"]/a/text()').extract()
        print("####################################")
        print("####################################")
        print("####################################")
        print(categ)
        ...
        if category is not None:
            ...
            name = response.xpath('.//div[@id="columna-derecha-2"]/div/h2/text()').re_first('\w.*')
            ...
            try:
                ...
                brand = None
            try:
                description = response.xpath('.//div[@class="product-description"]/div/p/text()').re_first('\w.*')
            except:
                print("error")
            category = category
            ...
            if Product_object.id:
                ...
                img_url = response.xpath('.//div[@id="columna-derecha-2"]/div/div/p/a/img/@src').re_first('\w.*')
