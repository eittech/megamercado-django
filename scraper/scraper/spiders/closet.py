...
class ProductSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            "https://closet.cl/semillas-closet/",
        ]
        for url in urls:
            response = scrapy.Request(url=url, callback=self.parse_category)
            response.meta['url_category_safe'] = "https://closet.cl/semillas-closet/"
            response.meta['name_category_safe'] = "semillas"
            response.meta['shop'] = 'https://closet.cl/'
            response.meta['shop_name'] = 'closet.cl'
            yield response

    def parse_category(self, response):
        list_product = response.css("figure.post-image a")
    ...
    def parse_product(self,response):
        ...
        if True:
            ...
            try:
                ...
                brand = None
            description = response.xpath('.//div[@class="woocommerce-product-details__short-description"]/p/text()').re_first('\w.*')
            category = category
