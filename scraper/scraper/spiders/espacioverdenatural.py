...
class ProductSpider(scrapy.Spider):
    name = "espacioverdenatural"

    def start_requests(self):
        urls = [
            "http://espacioverdenatural.cl/productos/",
        ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        categorias = response.css("div#woocommerce_product_categories-10 ul.product-categories li")
        for link_categoria in categorias:
            ...
            if url_category is not None:
                ...
                response.meta['shop'] = 'http://espacioverdenatural.cl/'
                response.meta['shop_name'] = 'espacioverdenatural.cl'
                yield response
    ...
    def parse_category(self, response):
        ...
        list_product = response.css("div.main-products-loop div.products-grid a.product-content-image")
        pagina_siguiente_css = response.css("nav.pagination-cubic a.next")
    ...
    def parse_product(self,response):
        ...
        if category is not None:
            ...
            try:
                description = ""
                description1 = response.css('div#content_tab_description div.tab-content-inner div.tab-content-scroll').extract_first()
            ...
            if Product_object.id:
                list_img = response.css("a.woocommerce-main-image")
                headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.3'}
                contador = 0
