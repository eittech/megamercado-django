...
def home(request):
    ...
    pk_product = servicecontractshop_ci.values('product__pk')
    # productos_ci = Product.objects.filter(pk__in=pk_product)
    productos_cd = Product.objects.filter(photo=True).filter(category__isnull=False).order_by('?')[:24]
    productos_ur = Product.objects.filter(photo=True).filter(category__isnull=False).order_by('?')[:24]
    productos_ci = Product.objects.filter(photo=True).filter(category__isnull=False).order_by('?')[:24]
